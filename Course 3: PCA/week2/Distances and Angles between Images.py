...
Write programs to compute distance.
Write programs to compute angle.
...
np.arccos((np.dot(x0,x1)/(np.dot(x0,x0)*np.dot(x1,x1))**(0.5)))
# GRADED FUNCTION: DO NOT EDIT THIS LINE
​
def distance(x0, x1):
    """Compute distance between two vectors x0, x1 using the dot product"""
    distance = np.sqrt(np.dot(x1-x0, x1-x0)) # <-- EDIT THIS to compute the distance between x0 and x1
    return distance
​
def angle(x0, x1):
    """Compute the angle between two vectors x0, x1 using the dot product"""
    angle = np.arccos( np.dot(x0,x1) / np.sqrt(np.dot(x0,x0)*np.dot(x1,x1)) ) # <-- EDIT THIS to compute angle between x0 and x1
    return angle
​
...
As a motivating example, consider the iris flower dataset. The dataset consists of 150 data points where each data point is a feature vector x∈ℝ4 describing the attribute of a flower in the dataset, the four dimensions represent

sepal length in cm
sepal width in cm
petal length in cm
petal width in cm
and the corresponding target y∈ℤ describes the class of the flower. It uses the integers 0, 1 and 2 to represent the 3 classes of flowers in this dataset.
