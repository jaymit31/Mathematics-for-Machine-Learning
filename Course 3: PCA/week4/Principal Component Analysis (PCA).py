...
Write code that implements PCA for high-dimensional datasets
...
Subtract from each image the mean μ.
Scale each dimension of each image by 1σ where σ is the stardard deviation.
...
1. PCA
Now we will implement PCA. Before we do that, let's pause for a moment and think about the steps for performing PCA. Assume that we are performing PCA on some dataset X for M principal components. We then need to perform the following steps, which we break into parts:

Data normalization (normalize).
Find eigenvalues and corresponding eigenvectors for the covariance matrix S. Sort by the largest eigenvalues and the corresponding eigenvectors (eig).
...
Compute the matrix XXT (a N by N matrix with N<<D)
Compute eigenvalues λs and eigenvectors V for XXT
Compute the eigenvectors for the original covariance matrix as XTV. Choose the eigenvectors associated with the M largest eigenvalues to be the basis of the principal subspace U.
Compute the orthogonal projection of the data onto the subspace spanned by columns of U. Functions you wrote for earlier assignments will be useful.
# GRADED FUNCTION: DO NOT EDIT THIS LINE
...
We first benchmark the time taken to compute XTX and XXT. Jupyter's magic command %time is quite handy.

The next cell finds the running time for computing $X^TX$ and XXT for different dimensions of X.
