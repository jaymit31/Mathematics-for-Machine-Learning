...
The algorithm is the same as you encountered in Gradient descent in a sandpit but this time instead of descending a pre-defined function, we shall descend the  χ2χ2  (chi squared) function which is both a function of the parameters that we are to optimise, but also the data that the model is to fit to.
...
Next recall that steepest descent shall move around in parameter space proportional to the negative of the Jacobian, i.e., [δμδσ]∝−J[δμδσ]∝−J, with the constant of proportionality being the aggression of the algorithm.

Modify the function below to include the ∂(χ2)∂σ∂(χ2)∂σ term of the Jacobian, the ∂(χ2)∂μ∂(χ2)∂μ term has been included for you.
